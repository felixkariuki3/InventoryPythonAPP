...
from backend.services.sales.reservations import ReservationService
...
def create_reservation(reservation_in: ReservationCreate, db: Session = Depends(get_db)):
    return ReservationService.create_reservation(db, reservation_in)
...
def update_reservation(reservation_id: int, reservation_in: ReservationUpdate, db: Session = Depends(get_db)):
    return ReservationService.update_reservation(db, reservation_id, reservation_in)
